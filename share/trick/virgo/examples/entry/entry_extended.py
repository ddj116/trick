...
class EntryDataPlayback(VirgoDataPlayback):
    ...
    def __init__(self, run_dir, scene, verbosity=1, headless=False,
        ...
        super().__init__(run_dir=run_dir, scene=scene, verbosity=verbosity,
                         headless=headless, images_dir=images_dir, video_filename=video_filename, splash=splash)
        pass
    def initialize(self):
        self.scene['frames']['landing_pos'] = {}
        super().initialize()
        self.get_landing_position()
        self.create_altitude_texts()

    def get_landing_position(self):
        positions = self.vdl.get_alias_datas(alias=
             self.scene['actors']['satellite']['driven_by']['pos'])
        self.landing_position=positions[-2] # pos @ 2nd to last timestep

    def create_altitude_texts(self):
           self.altitude_texts = {}
           layers = ['Thermosphere', 'Mesosphere', 'Stratosphere', 'Troposphere']
           for l in layers:
             self.altitude_texts[l] = VirgoLabel(name=l,text=l)
             self.altitude_texts[l].set_text("AW HELL YEA")
             self.altitude_texts[l].set_position(self.landing_position)
             self.altitude_texts[l].set_scale(1e6)
             self.altitude_texts[l].enable()
             self.renderers['foreground'].AddActor(self.altitude_texts[l].get_follower())
             # Welp I can't figure out why I can't put a virgo label at the landing
             # position and I'm sure I'm missing something super simple
